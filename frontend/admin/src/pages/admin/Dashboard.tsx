  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Calculate stats
        const occupied = plots.filter((p: any) => p.status === "occupied").length; // Assuming status exists
        const pending = tasks.filter((t: any) => t.status === "pending" || t.status === "in-progress").length;
        const tracked = plots.filter((p: any) => p.current_plant_id).length; // Assuming current_plant_id exists
        
        setStats({
          occupiedPlots: occupied,
          activeMembers: membersData.total || membersData.members.length,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Link
          to="/admin/gallery" // Assuming plants/catalogue is gallery or similar
          className="bg-white rounded-xl p-6 border border-[#E0E0E0] hover:shadow-lg transition-all group"
        >
          <div className="flex items-center justify-between">
            <div>
